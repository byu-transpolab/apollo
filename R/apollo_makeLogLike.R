...
apollo_makeLogLike <- function(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs, 
  ...
  if(apollo_inputs$apollo_control$analyticGrad){
    defs <- apollo_varList(apollo_probabilities, apollo_inputs)
    env  <- new.env(parent=baseenv())
    env$apollo_inputs <- apollo_inputs
    apollo_probabilities <- apollo_expandLoop(apollo_probabilities, defs, env)
    rm(defs, env)
  }
  ...
  if(apollo_inputs$apollo_control$analyticGrad){
    ...
    if(is.null(ll)){
      ...
      tmp3 <- tryCatch(apollo_probabilities(apollo_beta, apollo_inputs), error=function(e) NULL)
      test <- !is.null(tmp3) && is.numeric(tmp3) && is.numeric(ll) && length(tmp3)==length(ll)
      test <- test && abs(sum(tmp3, na.rm=TRUE) - sum(ll, na.rm=TRUE))/sum(tmp3, na.rm=TRUE) < 0.01
      if(test){
        apollo_probabilities <- tmp1
        if(exists('tmp2', inherits=FALSE)) apollo_inputs$apollo_randCoeff <- tmp2
      }
    }
    rm(ll, tmp1, test); if(exists('tmp2', inherits=FALSE)) rm(tmp2); if(exists('tmp3')) rm(tmp3)
  }
